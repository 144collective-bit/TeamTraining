...
export async function saveDraft(
  ...
  try {
    ...
    return await asTenant(user.tenantId, async (tx) => {
      ...
      const rev = await loadDraft(tx, user.tenantId, revisionId);
...
async function loadDraft(tx: Tx, tenantId: string, revisionId: string) {
  const [row] = await tx
    .select({
...
export async function publishRevision(
  ...
  try {
    ...
    return await asTenant(user.tenantId, async (tx) => {
      ...
      const affected = await publishAndCascade(tx, {
 ...
 */
async function publishAndCascade(tx: Tx, a: CascadeArgs): Promise<number> {
  return (async () => {
